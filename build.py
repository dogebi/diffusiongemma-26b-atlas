...
REWRITES: list[tuple[str, str]] = [
 ...
 R("REPORTED EVALUATIONS / NATIVE MODEL",
   ' renderBench(){const s=this.state,b=BENCH[s.bench];return html`<div><div class="inspect-path">PUBLISHER-REPORTED EVALUATIONS</div><h2 class="editorial">Capability,<br/><em>with the baseline named.</em></h2><p class="lede">Scores copied from the model card, next to the Gemma 4 26B A4B column it is compared with. No projection is invented here.</p><label class="field-label">Compare a benchmark<select value=${s.bench} onChange=${e=>this.patch({bench:Number(e.target.value)})}>${BENCH.map((r,i)=>html`<option value=${i}>${r.name}</option>`)}</select></label><div class="benchmark-chart">${BENCH_MODELS.map((name,i)=>html`<div class=${\'benchmark-row\'+(i===0?\' featured\':\'\')}><div><span>${name}</span><b>${b.values[i].toFixed(1)}</b></div><div class="bench-track"><i style=${{width:b.values[i]+\'%\',background:i===0?COL.dec:COL.enc,opacity:i===0?1:.32}}/></div></div>`)}</div><p class="fine">${b.name} / ${b.capability}. Percentages on a 0-100 scale as the card reports them. These are publisher-reported numbers, not results reproduced for this page, and they say nothing about the quantized builds.'),
